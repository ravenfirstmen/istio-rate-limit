...
Istio sideccar global rate limit
...
```shell
kubectl apply -f 01-deploy-http-bin.yaml
kubectl apply -f 02-envoy-filters.yaml
kubectl apply -f 03-rate-limit-configs.yaml
```
...
```shell
kubectl logs httpbin-54d78ffd98-hr25s -n httpbin -c "istio-proxy"
...
```